...
class DET extends Segment implements SegmentInterface
{
    ...
    public function setMsg(Msg $msg): void
    {
        $this
            //set name
            ->setData($msg->order->dt_of_observation?->format("y"), 1)
            ->setData($msg->order->dt_of_observation?->format("m"), 1, 1)
            ->setData($msg->order->dt_of_observation?->format("d"), 1, 2)
            ->setData($msg->order->dt_of_observation?->format("H"), 2)
            ->setData($msg->order->dt_of_observation?->format("i"), 2, 1);
    }
